...
func (s *Service) GetDesiredState(ctx context.Context, clusterConfig ClusterConfig, providerChartSpecs []key.ChartSpec) ([]*v1alpha1.ChartConfig, error) {
	...
	for _, chartSpec := range chartSpecs {
		chartConfigCR, err := s.newChartConfig(ctx, clusterConfig, chartSpec)
		if err != nil {
...
func (s *Service) getConfigMapSpec(ctx context.Context, clusterConfig ClusterConfig, chartSpec key.ChartSpec) (*v1alpha1.ChartConfigSpecConfigMap, error) {
	if chartSpec.ConfigMapName == "" {
		// Return early. Nothing to do.
		return &v1alpha1.ChartConfigSpecConfigMap{}, nil
	}

	tenantK8sClient, err := s.getTenantK8sClient(ctx, clusterConfig)
	if err != nil {
		return nil, microerror.Mask(err)
	}
...
func (s *Service) newChartConfig(ctx context.Context, clusterConfig ClusterConfig, chartSpec key.ChartSpec) (*v1alpha1.ChartConfig, error) {
	configMapSpec, err := s.getConfigMapSpec(ctx, clusterConfig, chartSpec)
	if err != nil {
		return nil, microerror.Mask(err)
	}

	labels := newChartConfigLabels(clusterConfig, chartSpec.AppName, s.projectName)
