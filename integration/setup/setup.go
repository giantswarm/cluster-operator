...
func installResources(config Config, vpcPeerID string) error {
...

	{
		...
		err = config.Host.InstallStableOperator("aws-operator", "awsconfig", e2etemplates.AWSOperatorChartValues)
		if err != nil {
			return microerror.Mask(err)
		}
	}
	...
	{
		err = config.Host.InstallBranchOperator("cluster-operator", "awsclusterconfig", template.ClusterOperatorChartValues)
		if err != nil {
			return microerror.Mask(err)
		}
