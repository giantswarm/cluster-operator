...
func mainWithError() (err error) {
	...
	daemonCommand.PersistentFlags().String(f.Guest.Cluster.Azure.HostCluster.CIDR, "", "CIDR Range for Azure HostClusters.")
	daemonCommand.PersistentFlags().String(f.Guest.Cluster.Kubernetes.API.ClusterIPRange, "", "CIDR Range for Pods in cluster.")
	daemonCommand.PersistentFlags().String(f.Guest.Cluster.Network.CIDR, "", "CIDR Range for guest clusters.")
	...
	daemonCommand.PersistentFlags().String(f.Service.Storage.Kind, "", "Microstorage storage type. Either \"memory\" or \"crd\".")
