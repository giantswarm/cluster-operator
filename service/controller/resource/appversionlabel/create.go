...
func (r *Resource) EnsureCreated(ctx context.Context, obj interface{}) error {
	...
	{
		r.logger.LogCtx(ctx, "level", "debug", "message", fmt.Sprintf("finding optional apps for tenant cluster %#q", clusterConfig.ID))
		...
		r.logger.LogCtx(ctx, "level", "debug", "message", fmt.Sprintf("found %d optional apps for tenant cluster %#q", len(apps), clusterConfig.ID))
	}

	{
		...
		if len(apps) > 0 {
			...
			r.logger.LogCtx(ctx, "level", "debug", "message", fmt.Sprintf("updating version label for optional apps in tenant cluster %#q", clusterConfig.ID))
			...
			r.logger.LogCtx(ctx, "level", "debug", "message", fmt.Sprintf("updating version label for %d optional apps in tenant cluster %#q", updatedAppCount, clusterConfig.ID))
		}
	}
