...
func (r Resource) EnsureCreated(ctx context.Context, obj interface{}) error {
	...
	r.logger.LogCtx(ctx, "level", "debug", "message", "Collecting all KVMClusterConfigs")

	rs, err := r.g8sClient.CoreV1alpha1().KVMClusterConfigs("").List(v1.ListOptions{})
	if err != nil {
		r.logger.LogCtx(ctx, "level", "error", "message", "could not get KVMClusterConfig resource", "stack", fmt.Sprintf("%#v", err))
		return microerror.Mask(err)
	}

	r.logger.LogCtx(ctx, "level", "debug", "message", fmt.Sprintf("Collected %d KVMClusterConfigs", len(rs.Items)))

	for _, kvmClusterConfig := range rs.Items {
		currentMap[kvmClusterConfig.Spec.Guest.ID] = true
	}
	...
	for _, cert := range certs.Items {
		...
		if _, ok := currentMap[clusterID]; !ok {
			...
			r.logger.LogCtx(ctx, "level", "debug", "message", fmt.Sprintf("CertConfig %#q do not have related KVMClusterConfigs, going to delete it", cert.Name))
			...
			r.logger.LogCtx(ctx, "level", "debug", "message", fmt.Sprintf("CertConfig %#q HAVE related KVMClusterConfigs, should not delete it", cert.Name))
		}
	}
